...
const realtorFees = (listing) => {
  let price = listing.price.replace("$", "").replace(",", "");
  return price <= 450000 ? price * .025 : price * .02;
}
 ...
 */
//WRITE YOUR CODE BELOW
let listingAscendingly = [...listings]
listingAscendingly.sort(
  (listingA, listingB) => listingA.price.replace("$", "").replace(",", "") - listingB.price.replace("$", "").replace(",", "")
);
...
//WRITE YOUR CODE BELOW
// my initial solution
let newBuiltListings = listings.filter(
  (listing) => listing.propertySummary.builtIn >= 1990
);
/*
listings.forEach((listing) => {
  console.log(listing.propertySummary.builtIn);
});
console.log("break");
newBuiltListings.forEach((listing) => {
  console.log(listing.propertySummary.builtIn);
});
*/

// Tasks-4-6 with regex
/* regex - Regular Expressions - https://www.w3schools.com/js/js_regexp.asp
 * a sequence  fo characters that forms a search pattern
 * 
 * Flags
 * /g - Global
 * /i - Case-insensitive
...
// example realtorFees(listings[2]) should return 16598
// example realtorFees(listings[6]) should return 10998
const realtorFeesRegex = (listing) => {
  let price = listing.price.replace(/\D/g, "");
  return price <= 450000 ? price * .025 : price * .02;
}
//console.log(realtorFeesRegex(listings[2]))
//console.log(realtorFeesRegex(listings[6]))

// Task-5
let listingAscendinglyRegex = [...listings];
listingAscendinglyRegex.sort(
  (listingA, listingB) => listingA.price.replace(/\D/g, "") - listingB.price.replace(/\D/g, "")
);
//console.log(listings);
//console.log(listingAscendinglyRegex);

// Task-6
let newBuildListingsRegex = listings.filter(
...
console.log("break");
newBuildListingsRegex.forEach((listing) => {
  console.log(listing.propertySummary.builtIn);
});
*/
